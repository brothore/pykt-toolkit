...
def parse_filename(filename):
    ...
    规则：
    1. 从前面获取数据集名称 (第一个下划线前)。
    2. 从后面获取折数 (最后一个下划线到 '.log' 之间)。
    3. 剩余中间部分作为模型名称。
    
    例如: 'assist2009_DTransformer_fold_0.log' -> ('assist2009', 'DTransformer', 'fold_0')
    """
    ...
    if len(parts) < 3:
        # 文件名不符合预期的格式（至少应包含 '数据集_模型_fold_x'）
        return None, None, None

    # 1. 数据集名称：第一个部分
    dataset_name = parts[0]

    # 2. 折数：最后两个部分（假设折数总是 'fold_x' 格式）
    fold_name = '_'.join(parts[-2:])
    
    # 3. 模型名称：中间剩余部分
    # 从索引 1 到 倒数第 3 个索引
    model_name_parts = parts[1:-2]
    model_name = '_'.join(model_name_parts)
    
    if not model_name: # 检查模型名称是否为空（例如: assist2009__fold_0）
         return None, None, None
