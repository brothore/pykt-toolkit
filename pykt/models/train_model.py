...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    elif model_name in ["akt","extrakt","folibikt", "robustkt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx","lefokt_akt", "dtransformer", "fluckt",  "Transformer_template", "balance_akt", "deepseekv3", "qwen"]:
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
...
def model_forward(model, data, rel=None):
    ...
    if model_name in ["dkt"]:
        ...
        ys.append(y[:, 1:])
    elif model_name in ["akt","extrakt","folibikt", "robustkt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lefokt_akt", "fluckt",  "Transformer_template", "deepseekv3", "qwen"]:               
        y, reg_loss = model(cc.long(), cr.long(), cq.long())
        ys.append(y[:,1:])
