...
# 3) 直接装精确匹配的 wheel URL
#    --no-deps: 切断 mamba-ssm 2.3.1 对 triton>=3.5 / tilelang / quack-kernels 的传染,
#               防止 pip 顺手把 torch 又升到 2.12
CCONV_WHL="https://github.com/Dao-AILab/causal-conv1d/releases/download/v1.6.2/causal_conv1d-1.6.1+cu12torch2.8cxx11abiTRUE-cp312-cp312-linux_x86_64.whl"
MAMBA_WHL="https://github.com/state-spaces/mamba/releases/download/v2.3.1/mamba_ssm-2.3.1+cu12torch2.8cxx11abiTRUE-cp312-cp312-linux_x86_64.whl"

echo
echo "==[1/3]== 安装 causal-conv1d (预编译 wheel) ..."
pip install --no-input --no-deps "$CCONV_WHL"

echo
echo "==[2/3]== 安装 mamba-ssm (预编译 wheel, --no-deps) ..."
pip install --no-input --no-deps "$MAMBA_WHL"
