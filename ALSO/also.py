...
class ALSO(torch.optim.Optimizer):
    ...
    def _initialize_pi(self, n_groups: int, pi_init: Optional[Tensor], pi_reg: Optional[Tensor]) -> None:
        ...
        dtype = torch.float32
        if pi_init is None:
            self.pi = torch.ones(n_groups, requires_grad=False, dtype=dtype)
        else:
            self.pi = pi_init.detach().clone().to(dtype=dtype)
        self.pi = self.pi / self.pi.sum()

        if pi_reg is None:
            self.pi_reg = torch.tensor(1. / n_groups, requires_grad=False, dtype=dtype)
        else:
            self.pi_reg = pi_reg.detach().clone().to(dtype=dtype) / pi_reg.sum().to(dtype=dtype)


    def _get_param_device(self):
    ...
    def _move_pi_to_device(self, device):
        ...
        self.pi = self.pi.to(device=device, dtype=torch.float32)
        self.pi_reg = self.pi_reg.to(device=device, dtype=torch.float32)
        if self.__pi_intermediate is not None:
            self.__pi_intermediate = self.__pi_intermediate.to(device=device, dtype=torch.float32)
        if self.__prev_grads_pi is not None:
            self.__prev_grads_pi = self.__prev_grads_pi.to(device=device, dtype=torch.float32)
    ...
    def _descent_ascent_step(self, closure, groups_indexes) -> float:
        ...
        # Update pi
        pi_grad_compressed = (-losses.clone().detach()).to(dtype=torch.float32)
        pi_grad = torch.zeros(self.pi.shape, device=self.pi.device, dtype=torch.float32, requires_grad=False)
    ...
    def step(self, closure, groups_indexes) -> float:
        ...
        """
        device = None
        if isinstance(groups_indexes, torch.Tensor):
            device = groups_indexes.device
        elif hasattr(closure, 'device'):
            device = closure.device
        else:
            device = self._get_param_device()

        self._move_pi_to_device(device)

        if not isinstance(groups_indexes, torch.Tensor):
            groups_indexes = torch.tensor(groups_indexes, device=device)
