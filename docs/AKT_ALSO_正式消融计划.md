...
若旧总调度器在已完成评测后仍因启动前版本的错误码退出，`scripts/auto_schedule_akt_also_stages2_to5.sh` 会等待补齐的阶段 1 结束，再自动连续运行阶段 2–5。该调度器仅运行修复后代码的实验。

阶段 2–5 的自动调度使用 `task-spooler` (`tsp`) 私有队列 `logs/akt_also_tsp.socket`，并设置 `tsp -S 2`：最多两个训练配置并行。每个配置是独立 TSP job；批大小为 128 的配置仍单独运行以避免超过显存预算。
